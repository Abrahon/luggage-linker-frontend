...
export default function MyPackagesPage() {
  ...
  const pickupCityParam = searchParams.get("pickup_city") || "";
  ...
  // Prevent multiple auto-open triggers
  const hasAutoOpenedRef = useRef(false);

  // Fetch package list from API & Auto-open Create Modal if needed
  const fetchPackages = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getMyPackages();
      ...
      setPackageList(validList);

      // Auto-open modal if no packages exist OR if redirected with route parameters
      if (!hasAutoOpenedRef.current) {
        if (validList.length === 0 || pickupCityParam || destinationCityParam) {
          setIsFormOpen(true);
  ...
  }, [pickupCityParam, destinationCityParam]);

  useEffect(() => {
    fetchPackages();
  }, [fetchPackages]);

  // Construct initial formData for PackageFormModal (pre-fills cities from query params)
  const formInitialData = useMemo(() => {
    if (selectedPackage) return selectedPackage;
    if (pickupCityParam || destinationCityParam) {
      return {
        pickup_city: pickupCityParam,
        destination_city: destinationCityParam,
      } as any;
    }
    return null;
  }, [selectedPackage, pickupCityParam, destinationCityParam]);
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50/50 py-4 sm:py-8 px-3 sm:px-6 lg:px-8 antialiased text-slate-800">
      <div className="w-full space-y-6 px-4 sm:px-6 lg:px-1">
        ...
        <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-5 border-b border-slate-200">
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${
                  loading || isFetchingDetail ? "animate-spin text-amber-500" : ""
                }`}
              />
            </button>
            <button
        ...
        {/* Dynamic Stats Grid */}
        <PackageStatsCards onStatClick={(status) => console.log("Stat clicked:", status)} />
        ...
        {/* Form Modal (Auto-opened or Manual) */}
        {isFormOpen && (
          <PackageFormModal
            isOpen={isFormOpen}
            packageToEdit={formInitialData}
            onClose={() => {
              setIsFormOpen(false);
              setSelectedPackage(null);
            }}
