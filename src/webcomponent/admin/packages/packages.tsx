...
export const Packages = () => {
  ...
  // Open Review Modal
  const handleOpenReview = (packageId: string) => {
    setSelectedReviewPackageId(packageId);
    setIsReviewOpen(true);
  };

  // Reset pointer-events when modal closes to protect against Radix focus locks
  useEffect(() => {
    if (!viewingPackage && !isReviewOpen) {
      const timer = setTimeout(() => {
        document.body.style.pointerEvents = "auto";
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [viewingPackage, isReviewOpen]);
  ...
  // Handle Refresh when review modal completes action
  const handleRefreshData = () => {
    toast.success("Package review completed successfully.");
    fetchPackages();
  };
  ...
  const handleViewDetails = async (packageId: string) => {
    try {
      ...
      toast.error("Could not fetch full package details from server.");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6 py-8 md:px-6 px-4 w-full">
                      ...
                      <DropdownMenu modal={false}>
                        ...
                        <DropdownMenuContent align="end" className="bg-white min-w-[170px]">
                          ...
                          <DropdownMenuItem
                            onSelect={(e) => {
                              e.preventDefault();
                              setTimeout(() => handleOpenReview(pkg.id), 50);
                            }}
                            className="cursor-pointer flex items-center gap-2"
                          >
      ...
      {/* Package Review Dialog Component */}
      <PackageReviewModal
