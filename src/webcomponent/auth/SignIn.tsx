...
import { Eye, EyeOff, Mail, Lock, AlertTriangle, ShieldAlert, LockKeyhole, CheckCircle2, ArrowRight } from "lucide-react";
...
interface AlertState {
  type: "success" | "destructive" | "warning" | null;
  title: string;
  message: string;
  actionUrl?: string;
  actionText?: string;
...
export const SignIn = () => {
  ...
  const onSubmit = async (data: SignInFormData) => {
    ...
    setAlertState({ type: null, title: "", message: "" });
    clearErrors();
    ...
    try {
      ...
      if (fieldErrors?.email) {
        ...
        const msg = emailErr.message || "Email validation error.";
        ...
        if (code === "email_not_verified") {
          setAlertState({
            ...
            message: msg,
            actionUrl: `/verify-email?email=${encodeURIComponent(getValues("email") || "")}`,
        ...
        // Standard Email Field Error (e.g. account_not_found)
      ...
      // =========================================================
      // 2. PASSWORD ERRORS (e.g. invalid_password)
      // =========================================================
      if (fieldErrors?.password) {
        const passErr = fieldErrors.password;
        const msg = passErr.message || "Incorrect password.";
      ...
      setAlertState({
        type: "destructive",
        title: "Login Failed",
        message: resData?.message || "Invalid credentials or server error.",
      });
    }
  };
        ...
        {alertState.type && (
          ...
          >
            ...
            <div className="flex justify-between items-center w-full">
              ...
              {alertState.actionUrl && (
                <Link
                  href={alertState.actionUrl}
                  className="inline-flex items-center gap-1 px-3 py-1 text-xs font-bold bg-amber-400 text-slate-950 rounded hover:bg-amber-300 transition-colors shrink-0 ml-3"
                >
